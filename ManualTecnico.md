...
- **Línea 20:** `$router->post(pattern, handler)` registra una ruta POST para login
  ...
  - **Ruta pública:** No requiere token JWT (está en `$publicRoutes` del AuthMiddleware)

- **Línea 21 (NUEVA):** `$router->post('/api/v1/auth/register', [AuthController::class, 'register'])`
  - Patrón: `/api/v1/auth/register` (exacto, sin parámetros dinámicos)
  - Handler: `[AuthController::class, 'register']` (array que indica clase y método)
  - **Ruta protegida:** Requiere token JWT válido + validación adicional de que el usuario sea admin
  - NO está en `$publicRoutes`, por lo que `AuthMiddleware` valida el token antes de ejecutar el handler
...
- **Línea 16-19:** Array `$publicRoutes` — Define rutas que NO requieren token. Útil para:
  ...
  - **Nota:** `/api/v1/auth/register` **NO** está en esta lista. Requiere token JWT válido + validación adicional en el controller (solo admins pueden crear usuarios)
